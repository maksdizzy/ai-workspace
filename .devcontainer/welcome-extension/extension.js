...
async function loginViaHost() {
  const LOGIN_CMD = 'pip install "notebooklm-py[browser]" && playwright install chromium && notebooklm login';
  ...
  const step1 = await vscode.window.showInformationMessage(
    ...
    'Copy Command', 'I already have credentials'
  );
...
async function importCredentials() {
  ...
  const step2 = await vscode.window.showInformationMessage(
    ...
    'Copy Command', 'Paste Credentials'
  );
...
async function pasteCredentials() {
  ...
  try {
    ...
    vscode.window.showErrorMessage('Invalid JSON. Please copy the full contents of storage_state.json.');
  }
}
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ai-workspace.notebooklmLogin', async () => {
      if (isLoggedIn()) {
        vscode.window.showInformationMessage(
          'NotebookLM is already connected! Try: notebooklm notebooks list'
        );
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ai-workspace.pasteCredentials', pasteCredentials)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ai-workspace.notebooklmCheck', () => {
      if (isLoggedIn()) {
        ...
        terminal.show();
        terminal.sendText('notebooklm notebooks list');
      } else {
